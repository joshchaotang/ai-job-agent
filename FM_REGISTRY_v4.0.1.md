...
**觸發場景**：
- 提供部署指令前，未確認目標服務當前可達性
- Vercel、GitHub、Netlify 等平台可能有服務中斷
- 導致用戶執行後卡住，浪費時間
...
1. Git push 成功 ✅
2. 開啟 Vercel 網頁準備部署
3. 網頁載入超過 10 分鐘，持續「載入中...」
4. 無法完成部署，用戶困惑

AI 提供的步驟：
❌ 直接說「去 Vercel 網頁點 Import」
❌ 沒有先檢查 Vercel 服務狀態
❌ 沒有提供 CLI 備援方案
❌ 沒有提供「如果卡住」的應對指引
```
...
**根本原因**：
1. 未做「管道可行性檢查」（違反 FM-003 精神）
2. 假設平台永遠可用
3. 沒有提供備援方案
4. 用戶體驗差（卡住不知怎麼辦）

**防護措施**：
1. ✅ 部署前先用 WebSearch 檢查服務狀態
2. ✅ 預設提供 CLI 替代方案作為備援
3. ✅ 提供故障排除步驟（清除快取、無痕模式）
4. ✅ 在部署說明中加入「如果網頁卡住」的應對指引
5. ✅ 優先推薦 CLI 方式（更穩定、更專業）
...
**品質判定**：
維度 × 權重 × 得分 = 總分
- 根因識別（30%）× 70分 = 21（識別出 4 個原因，但未實際測試）
- 方案完整性（30%）× 90分 = 27（提供 3 層方案）
- 可執行性（25%）× 80分 = 20（CLI 可用，網頁需測試）
- 預防機制（15%）× 60分 = 9（應該在交付前就做到）
**總分：77 / 100**
...
- 流程規範違反：FM-003, FM-004, FM-007, FM-009
- 測試覆蓋不足：FM-005
- 品質管理缺失：FM-006
- 環境認知錯誤：FM-008
